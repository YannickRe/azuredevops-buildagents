param(
    ...
    [String] [Parameter (Mandatory=$true)] $VmssNameWindows2025,
...
$windowsManagedImage2025 = az vmss show --name $VmssNameWindows2025 --resource-group $AgentsResourceGroup --query 'virtualMachineProfile.storageProfile.imageReference.id' --subscription $SubscriptionId | Out-String | ConvertFrom-Json
...
foreach ($managedImage in $managedImages) {
    if ($managedImage -ne $windowsManagedImage2022 -and $managedImage -ne $windowsManagedImage2025 -and $managedImage -ne $ubuntuManagedImage2204 -and $managedImage -ne $ubuntuManagedImage2404) {
